       ...
       DrawMaze.
       display " "
       perform varying MazeRow from 1 by 1 until mazerow is greater 
        than 8
       perform varying MazeColumn from 1 by 1 until MazeColumn is 
        greater than 8
       ...
       InitializeMaze.
       PERFORM VARYING MazeColumn FROM 1 BY 1 UNTIL MazeColumn
       IS GREATER THAN 8
       PERFORM VARYING MazeRow FROM 1 BY 1 UNTIL MazeRow IS GREATER
       THAN 8
       MOVE "O" TO State(MazeColumn,MazeRow)
       PERFORM VARYING Direction FROM 1 BY 1 UNTIL Direction IS greater 
       THAN 4
       ...
       IF NextColumn IS GREATER THAN 0 AND NextRow IS GREATER THAN 
        0 AND NextColumn IS NOT GREATER THAN 8 AND NextRow IS NOT 
        GREATER THAN 8 THEN
       MOVE "F" TO State(NextColumn, NextRow)
       END-IF
       END-PERFORM
       PERFORM WITH TEST AFTER UNTIL MazeGenComplete IS EQUAL TO "Y"
       PERFORM WITH TEST AFTER UNTIL State(MazeColumn, MazeRow) IS EQUAL
        TO "F"
       COMPUTE MazeColumn = FUNCTION RANDOM() * 8 + 1
       COMPUTE MazeRow = FUNCTION RANDOM() * 8 + 1
       END-PERFORM
       ...
       perform with test after until DoorCandidate(Direction) is equal 
        to "Y"
       ...
       IF NextColumn IS GREATER THAN 0 AND NextRow IS GREATER THAN 
        0 AND NextColumn IS NOT GREATER THAN 8 AND NextRow IS NOT 
        GREATER THAN 8 and state(NextColumn, NextRow) IS EQUAL TO "O" 
        THEN
       MOVE "F" TO State(NextColumn, NextRow)
       END-IF
       END-PERFORM
       ...
       perform DetermineNextPosition
       if NextColumn is less than 1 or nextrow is less than 1 or 
       nextcolumn is greater than 8 or nextrow is greater than 8
       then 
       ...
       PERFORM VARYING MazeColumn FROM 1 BY 1 UNTIL MazeColumn is 
        greater than 8
       perform varying MazeRow from 1 by 1 until mazerow is greater than 
        8
